window.addEventListener("load", function () {
  ...
  let s2 = new CleverSlider(".gallery-2");
  s2.initControls();
  s2.start(3000);
});
...
class CleverSlider extends Slider {
  interval = null;
...
class OtherSlider extends CleverSlider {
  toggleSlides(imgHide, imgShow, isNext) {
    this.animatedNow = true;

    imgHide.classList.remove("showed");
    let animHide = imgHide.animate(
      isNext ? this.animMoveToLeft : this.animMoveToRight,
      { duration: 500 }
    );
